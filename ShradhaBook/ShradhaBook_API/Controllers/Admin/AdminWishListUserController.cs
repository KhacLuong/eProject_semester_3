...
namespace ShradhaBook_API.Controllers.Admin
{
    ...
    public class AdminWishListUserController : ControllerBase
    {
        ...
        public AdminWishListUserController(IWishListUserService wishListUserService, IProductService productService, IMapper mapper)
        {
        ...
        public async Task<ActionResult<WishListUserGet>> GetWishListUser(int id)
        {
            try
            {
                ...
                return result == null ? NotFound(new MyServiceResponse<WishListUserGet>(false, MyStatusCode.NOT_FOUND_RESULT)) : Ok(new MyServiceResponse<WishListUserGet>(result));
        ...
        public async Task<IActionResult> UpdateWishListUser(int id, WishListUserPost model)
        {
            try
            {
                ...
                int status = await _wishListUserService.UpdateWishListUserAsync(id, model);
                if (status == MyStatusCode.DUPLICATE)
                {
                    return BadRequest(new MyServiceResponse<WishListUserGet>(false, MyStatusCode.UPDATE_FAILURE_RESULT + ", There is already a WishListUser of this WhishListId and this UserId"));
                }
                else if (status == MyStatusCode.NOTFOUND)
                {
                    return BadRequest(new MyServiceResponse<WishListGet>(false, MyStatusCode.ADD_FAILURE_RESULT + ", Not found User or Wishlist"));
        ...
        public async Task<ActionResult<WishListUserGet>> AddWishListUser(WishListUserPost model)
        {
            //try
            {
                ...
                if (status == MyStatusCode.DUPLICATE)
                {
                    return BadRequest(new MyServiceResponse<WishListUserGet>(false, MyStatusCode.ADD_FAILURE_RESULT + ", There is already a WishListUser of this WhishListId and this UserId"));
                }
                else if (status == MyStatusCode.NOTFOUND)
                {
                    return BadRequest(new MyServiceResponse<WishListGet>(false, MyStatusCode.ADD_FAILURE_RESULT + ", Not found User or Wishlist"));
