...
public class AdminWishListController : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<WishListGet>>> GetAllWishList()
    {
        ...
        catch
        {
            return StatusCode(500,
                new MyServiceResponse<List<WishListGet>>(false, MyStatusCode.INTERN_SEVER_ERROR_RESULT));
        }
    }
    ...
    public async Task<ActionResult<WishListGet>> GetWishList(int id)
    {
        try
        {
            ...
            return result == null
                ? NotFound(new MyServiceResponse<WishListGet>(false, MyStatusCode.NOT_FOUND_RESULT))
                : Ok(new MyServiceResponse<WishListGet>(result));
        }
        catch
        {
    ...
    public async Task<IActionResult> UpdateWishList(int id, WishListPost model)
    {
        try
        {
            var status = await _wishListService.UpdateWishListAsync(id, model);
    ...
    public async Task<ActionResult<WishListGet>> AddWishList(WishListPost model)
    {
        try
        {
            ...
            if (status > 0)
            {
                ...
                return Ok(new MyServiceResponse<WishListGet>(_mapper.Map<WishListGet>(newEntity), true,
    ...
    public async Task<IActionResult> DeleteWishListGet(int id)
    {
        try
        {
            ...
            if (result == MyStatusCode.EXISTSREFERENCE)
                return BadRequest(new MyServiceResponse<WishListGet>(false,
                    MyStatusCode.DELLETE_FAILURE_RESULT + ", " + "There are already WishListUser  of WishList"));
