...
public class WishListService : IWishListService
{
    ...
    {
        var checkExists = _context.WishLists.Any(w => w.ProductId == model.ProductId);
        if (checkExists) return MyStatusCode.DUPLICATE;
        if (!_context.Products.Any(p => p.Id == model.ProductId)) return MyStatusCode.NOTFOUND;
    ...
    public async Task<bool> checkExistsWishListByProductId(int id)
    {
        return false;
    }
    ...
    {
        var checkExistsReference = await _context.WishListUsers.AnyAsync(w => w.WishListId == id);
    ...
    {
        var allModel = await _context.WishLists!.ToListAsync();
