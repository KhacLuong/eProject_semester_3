...
public class ProductService : IProductService
{
    ...
    public async Task<int> AddProductAsync(ProductModelPost model)
    {
        ...
        if (model.Code.Length != 7 || !Helpers.Helpers.IsValidCode(model.Code)) return MyStatusCode.FAILURE;
        ...
        int pageSize = 20, int pageIndex = 1)
    {
        ...
        else if (sortBy == 5) query = query.OrderBy(m => m.UpdatedAt);
    ...
    public async Task<int> UpdateProductAsync(int id, ProductModelPost model)
    {
        if (id == model.Id)
        {
            ...
            if (model.Code.Length != 7 || !Helpers.Helpers.IsValidCode(model.Code)) return MyStatusCode.FAILURE;
            if (!model.Code.Substring(0, 2).Equals(categoty.Code.Substring(0, 2))) return MyStatusCode.FAILURE;
