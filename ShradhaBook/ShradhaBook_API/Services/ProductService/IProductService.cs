...
namespace ShradhaBook_API.Services.ProductService
{
    public interface IProductService
    {
        ...
        Task<List<ProductModel>> GetAllProductAsync(string? name, string? code, int? categoryId, int? manufactoryId, decimal? price, long quantity, int? status = 0, int? sortBy = 0);
