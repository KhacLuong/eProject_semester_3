...
public class CommentService : ICommentService
{
    ...
    {
        if (model.Content == null || model.Content.Trim().Length == 0) return MyStatusCode.FAILURE;
        var checkRateExists =
            await _context.Rates.SingleAsync(r => r.UserId == model.UserId && r.ProductId == model.ProductId);
        if (checkRateExists == null) return MyStatusCode.NOTFOUND_RATE;
        var checkCommentExists =
            await _context.Comments.AnyAsync(r => r.UserId == model.UserId && r.ProductId == model.ProductId);
        if (!checkCommentExists) return MyStatusCode.FAILURE;
    ...
    public async Task<int> UpdateCommentAsync(int id, CommentModelPost model)
    {
        if (id == model.Id)
        {
            ...
            var checkRateExists =
                await _context.Rates.SingleAsync(r => r.UserId == model.UserId && r.ProductId == model.ProductId);
            if (checkRateExists == null) return MyStatusCode.FAILURE;
            var checkCommentExists = await _context.Comments.AnyAsync(r =>
