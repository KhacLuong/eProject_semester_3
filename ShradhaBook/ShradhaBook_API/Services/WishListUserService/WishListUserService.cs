...
public class WishListUserService : IWishListUserService
{
    ...
    {
        var wishLispost = new WishListPost(0, productId, null, null);
        ...
        if (checkExists) return MyStatusCode.DUPLICATE;
        var modelPost = new WishListUserPost(0, userId, wishList.Id, null, null);
    ...
    public async Task<object> GetCountWishListAndCart(int userId)
    {
        ...
        var totalOrderIsDone = _context.Orders
            .Where(o => o.UserId == userId && o.OrderTracking.Equals(MyStatusCode.ORDER_DONE)).Count();
    ...
    public async Task<int> UpdateWishListUserAsync(int id, WishListUserPost model)
    {
        if (id == model.Id)
        {
            if (_context.WishListUsers.Any(c =>
                    c.WishListId == model.WishListId && c.UserId != model.UserId && c.Id != model.Id))
                return MyStatusCode.DUPLICATE;
            if (!_context.WishLists.Any(w => w.Id == model.WishListId) ||
                !_context.Users.Any(u => u.Id == model.UserId)) return MyStatusCode.NOTFOUND;
